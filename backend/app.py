...
def content_filtering():
    try:
        ...
        # Filter nilai cosine similarity (tanpa 0)
        cosine_details = [(i, val) for i, val in enumerate(cosine_sim) if val > 0]

        return jsonify({
            'diagnosis': diagnosis_text,
            'cosine_similarity_scores': cosine_details,  # Detail nilai cosine similarity
            'cosine_similarity': cosine_sim[top_indices[0]],  # Skor tertinggi
            'top_matches': top_matches  # Diagnosis dengan skor tertinggi
        }), 200
    except Exception as e:
...
def get_diagnosis():
    try:
        ...
        if query:
            ...
            diagnosis_list = diagnosis_df['Diagnosis'].tolist()
        return jsonify(diagnosis_list)
    except Exception as e:
        logging.error(f"Error in get_diagnosis: {e}")
        return jsonify({'error': str(e)}), 500
...
def get_recommendations(diagnosis_text):
    try:
        ...
        cosine_sim_diagnosis = cosine_similarity(tfidf_diagnosis, normalize(tfidf_matrix)).flatten()
        ...
        # Filter TF-IDF dan Cosine Similarity untuk menghilangkan nilai 0
        tfidf_nonzero = [(i, val) for i, val in enumerate(tfidf_diagnosis.toarray().flatten()) if val != 0]
        cosine_nonzero = [(i, val) for i, val in enumerate(cosine_sim_diagnosis) if val != 0]

        # Return matched diagnoses dan recommended medicines
        return {
            'matched_diagnoses': matched_diagnoses['Diagnosis'].tolist(),
            'Resep Obat': recommended_obat_list,
            'tf_idf_vector': tfidf_nonzero,  # TF-IDF tanpa 0
            'cosine_similarity_scores': cosine_nonzero  # Skor Cosine tanpa 0
        }
    except Exception as e:
